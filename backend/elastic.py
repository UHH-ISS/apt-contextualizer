...
class Client:
    ...
    def __init__(self, host, port, indices, credentials: tuple[str, str]):
        ...
        print("Connected to ES node %s prepared" % host)

        for index in indices:
            self.es.indices.create(index=index, ignore=400)

        # self.es.indices.create(index='weak_aggregation', body=mapping_weak_aggregation, ignore=400)
        # self.es.indices.create(index='alerts_pattern', body=mapping_meta_alerts, ignore=400)
        # Meta Alerts probably not needed
    ...
    def __wrap_alert_generator_to_elaticsearch_actions(self, alert_generator, index):
